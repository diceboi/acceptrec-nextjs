...
export default function ForCandidates() {
  ...
  const categoryFromQuery = ["Candidates", "Career advice"];
  
  const filteredPosts = categoryFromQuery.length > 0
    ? postsdata?.posts.edges.filter(
        (post) => categoryFromQuery.includes(post.node.categories.nodes[0].name)
      )
